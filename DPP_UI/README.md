...
Use `NEXT_PUBLIC_DPP_LEGACY_CREATE=1` when your on-chain package still uses the older `create_return_id` (only GTIN, material, reward, recipient). Omit it or set `0` after you republish `DPP_core` with country + other-metadata fields on create.
