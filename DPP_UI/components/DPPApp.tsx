...
const DPPApp = () => {
  ...
  async function handleCreateDPP() {
    ...
    if (!gtin.trim()) { alert("Please enter a GTIN (product identifier)"); return; }

    setCreating(true);
    const result = usePTB
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ? await storage.createDPPWithPTB(gtin, material, metadata.countryOfManufacture, metadata.otherMetadata, lockedReward, currentAccount.address, signAndExecute as any)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      : await storage.createDPP(gtin, material, metadata.countryOfManufacture, metadata.otherMetadata, lockedReward, currentAccount.address, signAndExecute as any);
  ...
  async function handleLookupGTIN() {
    if (!lookupGtin.trim()) { alert("Please enter a GTIN to lookup"); return; }
    setLooking(true);
    const dpp = await storage.getDPPByGTIN(lookupGtin);
    if (dpp) { setCurrentDPP(dpp); setActiveTab("consumer"); }
    else alert("GTIN not found in registry: " + lookupGtin);
    setLooking(false);
  }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.card}>
        {activeTab === "manufacturer" && (
          <ManufacturerView
            gtin={gtin} setGtin={setGtin}
            material={material} setMaterial={setMaterial}
            metadata={metadata} setMetadata={setMetadata}
            metadataOpen={metadataOpen} setMetadataOpen={setMetadataOpen}
            lockedReward={lockedReward} setLockedReward={setLockedReward}
            usePTB={usePTB} setUsePTB={setUsePTB}
            creating={creating}
        ...
        {activeTab === "registry" && (
          <RegistryView
            lookupGtin={lookupGtin} setLookupGtin={setLookupGtin}
            looking={looking} allDPPs={allDPPs}
            onLookup={handleLookupGTIN}
            onSelectDPP={(dpp) => { setCurrentDPP(dpp); setActiveTab("consumer"); }}
          />
        )}
        {activeTab === "consumer" && (
          <ConsumerView
      ...
      <div className={styles.footer}>
        ...
        <p className={styles.footerLine}>Tabulas - Circular Economy Infrastructure</p>
      </div>
    </div>
  );
};
