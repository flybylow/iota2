...
interface Tab {
  id: 'producer' | 'consumer' | 'recycler';
  label: string;
  color: string;
}

const TShirtEscrow = () => {
  const [activeTab, setActiveTab] = useState<'producer' | 'consumer' | 'recycler'>('producer');
  ...
  useEffect(() => {
    console.log('═══════════════════════════════════════');
    console.log('🎬 T-SHIRT ESCROW APP INITIALIZED');
    console.log('═══════════════════════════════════════');
    console.log('App Version: Mock Prototype (Phase 1)');
    console.log('Network: Local (Mock State)');
    console.log('Initial State:');
    console.log('  Active Tab:', activeTab);
    console.log('  DPP:', dpp || 'none');
    console.log('  Material:', material);
    console.log('  Locked Value: €', lockedValue.toFixed(2));
    console.log('Status: Ready for demo ✅');
    console.log('═══════════════════════════════════════\n');
    console.log('💡 TIP: All actions will be logged here in the console');
    console.log('');
  }, []);

  // 📊 State Change Logger - DPP
  useEffect(() => {
    if (dpp) {
      console.log('📊 STATE UPDATE: DPP');
      console.log('  DPP ID:', dpp.id);
      console.log('  Status: Active');
    } else {
      console.log('📊 STATE UPDATE: DPP');
      console.log('  Status: No DPP');
    }
  }, [dpp]);

  // 📊 State Change Logger - End of Life
  useEffect(() => {
    console.log('📊 STATE UPDATE: End of Life Status');
    console.log('  Marked:', endOfLifeMarked ? '✅ Yes' : '❌ No');
  }, [endOfLifeMarked]);

  // 📊 State Change Logger - Unlock Status
  useEffect(() => {
    console.log('📊 STATE UPDATE: Value Unlock Status');
    console.log('  Unlocked:', unlocked ? '✅ Yes (Consumer can claim)' : '🔒 No (Still locked)');
  }, [unlocked]);

  // 🎯 Enhanced Tab Change Handler
  const handleTabChange = (tabId: 'producer' | 'consumer' | 'recycler') => {
  ...
  const handleValueChange = (newValue: number) => {
    ...
    console.log('  Previous: €', lockedValue.toFixed(2));
    console.log('  New: €', newValue.toFixed(2));
    console.log('  Timestamp:', new Date().toISOString());
  ...
  const createDPP = () => {
    console.log('═══════════════════════════════════════');
    console.log('🏭 PRODUCER: CREATE DPP');
    console.log('═══════════════════════════════════════');
    console.log('Input:');
    console.log('  Material:', material);
    console.log('  Locked Value: €', lockedValue.toFixed(2));
    ...
    console.log('Output:');
    console.log('  DPP ID:', newDPP.id);
    console.log('  Producer:', newDPP.producer);
    console.log('  Created At:', newDPP.createdAt);
  ...
  const handleWalletChange = (address: string) => {
    console.log('👛 WALLET ADDRESS CHANGE');
    console.log('  Previous:', consumerWallet || '(empty)');
    console.log('  New:', address || '(empty)');
    console.log('  Length:', address.length);
    console.log('  Valid:', address.trim().length > 0 ? '✅' : '❌');
    setConsumerWallet(address);
  };

  // 🎲 Enhanced Demo Wallet Handler
  const useDemoWallet = () => {
    const demoAddress = '0xA1b2C3d4E5f6789012345678901234567890abcd';
    console.log('═══════════════════════════════════════');
    console.log('🎲 USE DEMO WALLET');
    console.log('═══════════════════════════════════════');
    console.log('  Demo Address:', demoAddress);
    console.log('  Action: Auto-fill wallet field');
    console.log('  Status: Demo wallet applied ✅');
    console.log('═══════════════════════════════════════\n');
    setConsumerWallet(demoAddress);
  };

  // 🏁 Enhanced Mark End of Life
  const markEndOfLife = () => {
    console.log('═══════════════════════════════════════');
    console.log('👤 CONSUMER: MARK END OF LIFE');
    console.log('═══════════════════════════════════════');
    console.log('Validation:');
    console.log('  DPP Exists:', dpp ? '✅' : '❌');
    console.log('  Wallet Address:', consumerWallet);
    console.log('  Wallet Valid:', consumerWallet.trim() ? '✅' : '❌');
    
    if (consumerWallet.trim()) {
      console.log('Action:');
      console.log('  Status Change: In Use → Pending Recycler');
      console.log('  Consumer Wallet Registered:', consumerWallet);
      console.log('  Awaiting recycler verification...');
  ...
  const confirmRecycle = () => {
    ...
    console.log('  DPP ID:', dpp?.id);
    console.log('  Producer Claim:', dpp?.material);
    console.log('  Locked Value: €', dpp?.lockedValue.toFixed(2));
    console.log('  Consumer Wallet:', consumerWallet);
    console.log('Decision:');
    console.log('  Material Match: ✅ VERIFIED');
    console.log('Action:');
    console.log('  Unlocking value: €', dpp?.lockedValue.toFixed(2));
    console.log('  Transferring to:', consumerWallet);
    console.log('  Status: Value Unlocked Successfully ✅');
    console.log('  Consumer can claim reward! 🎉');
    console.log('═══════════════════════════════════════\n');
    setUnlocked(true);
  };

  // ⚠️ Enhanced Flag Mismatch
  const flagMismatch = () => {
    console.log('═══════════════════════════════════════');
    console.log('⚠️ RECYCLER: FLAG MATERIAL MISMATCH');
    console.log('═══════════════════════════════════════');
    console.log('Verification:');
    console.log('  DPP ID:', dpp?.id);
    console.log('  Producer Claim:', dpp?.material);
    console.log('  Actual Material: [Recycler would specify]');
    console.log('Decision:');
    console.log('  Material Match: ❌ MISMATCH DETECTED');
    console.log('Action:');
    console.log('  Producer Flagged for Greenwashing');
    console.log('  Value Remains Locked');
    console.log('  On-chain Record Created');
    console.log('  Status: Greenwashing Flagged ⚠️');
    console.log('═══════════════════════════════════════\n');
    // In production, this would update the DPP status to "flagged"
  };
  ...
  const resetDemo = () => {
    ...
    console.log('  DPP:', dpp?.id || 'none');
    console.log('  Wallet:', consumerWallet || 'empty');
    console.log('  EOL Marked:', endOfLifeMarked);
    console.log('  Unlocked:', unlocked);
    console.log('Action:');
    console.log('  Clearing all state...');
    console.log('  Resetting to initial state...');
    console.log('  Status: Demo Reset Complete ✅');
    console.log('═══════════════════════════════════════\n');
